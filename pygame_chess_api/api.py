...
class Piece:
    ...
    def case_allowed(self, case_pos: tuple, this_move_can_kill=True,
                     skip_check_verification=False) -> Move:  # returns move
        ...
        def verify_check(skip_check_verification):
            ...
            if self.board.check_pieces[self.board.cur_color_turn].in_check_situation(hypothesis=hypo_board):
                return Move(Move.LEADING_TO_CHECK_SITUATION_MOVE, self,
                            (-1, -1))  # not allowed because would be in check situation
            return None

        to_promotion_pawn = (isinstance(self, Pawn) and (
                    (self.color == self.WHITE and case_pos[1] == 0) or (self.color == self.BLACK and case_pos[1] == 7)))
    ...
    def move(self, pos_or_move: tuple or Move, skip_allowed_verif=False, call_new_turn=True) -> None or tuple:
        ...
        return self.board.move_piece(self, pos_or_move, skip_allowed_verif=skip_allowed_verif,
                                     call_new_turn=call_new_turn)
        ...
        for piece in board.pieces_by_pos.values():
            if piece.color != self.color:
                if piece.invicible:  # to avoid recursive erros
                    if sqrt((self_in_hypothesis.pos[0] - piece.pos[0]) ** 2 + (
                            self_in_hypothesis.pos[1] - piece.pos[1]) ** 2) < 2:
    ...
    def get_moves_allowed(self, skip_check_verification=False):  # returns every move allowed
        ...
        if not self.has_already_moved:
            ...
            for cur_pos in little_castling_free_pos_needed:
                if cur_pos in self.board.pieces_by_pos or (not skip_check_verification and self.in_check_situation(
                        self.board.create_hypothesis_board({self: cur_pos}))):
                    pos_little_castling_are_free = False

            piece_at_rook_needed_pos = self.board.pieces_by_pos[(self.pos[0] + 3, self.pos[1])] if (self.pos[0] + 3,
                                                                                                    self.pos[
                                                                                                        1]) in self.board.pieces_by_pos else None
            if pos_little_castling_are_free and type(
                    piece_at_rook_needed_pos) == Rook and not piece_at_rook_needed_pos.has_already_moved:
                if skip_check_verification or not self.in_check_situation(
                        self.board.create_hypothesis_board({self: (self.pos[0] + 2, self.pos[1])})):
                    moves_around.append(
                        Move(Move.SPECIAL_MOVE, self, (self.pos[0] + 2, self.pos[1]), Move.CASTLING_TYPE))
            ...
            piece_at_rook_needed_pos = self.board.pieces_by_pos[(self.pos[0] - 4, self.pos[1])] if (self.pos[0] - 4,
                                                                                                    self.pos[
                                                                                                        1]) in self.board.pieces_by_pos else None
            if pos_big_castling_are_free and type(
                    piece_at_rook_needed_pos) == Rook and not piece_at_rook_needed_pos.has_already_moved:
                if skip_check_verification or not self.in_check_situation(
                        self.board.create_hypothesis_board({self: (self.pos[0] - 2, self.pos[1])})):
                    moves_around.append(
...
class Queen(Piece):
    ...
    def get_moves_allowed(self, skip_check_verification=False):  # returns every move allowed
        return self.cases_allowed_around(skip_check_verification) + self.cases_allowed_in_diagonals(
            skip_check_verification) + self.cases_allowed_in_line(
            skip_check_verification)  # may be duplications but we don't matter
...
class Pawn(Piece):
    ...
    def get_moves_allowed(self, skip_check_verification=False):  # returns every move allowed
        ...
        in_front_move = self.case_allowed(in_front_case_pos, this_move_can_kill=False,
                                          skip_check_verification=skip_check_verification)
        ...
        if ((self.color == self.WHITE and self.pos[1] == 3) or (self.color == self.BLACK and self.pos[1] == 4)) and len(self.board.move_history) > 0:
            ...
            en_passant_pos = (piece_of_last_move.pos[0], piece_of_last_move.pos[1] + (
                        last_move_history["ini_pos"][1] - piece_of_last_move.pos[1]) // 2)
            if type(piece_of_last_move) == __class__ and piece_of_last_move.pos[1] == self.pos[1] and abs(
                    piece_of_last_move.pos[0] - self.pos[0]) == 1 and \
                    not last_move_history["has_already_moved"] and self.case_allowed(en_passant_pos,
                                                                                     skip_check_verification=skip_check_verification):
...
class Board:
    ...
    def move_piece(self, piece, pos_or_move: tuple or Move, skip_allowed_verif=False,
                   call_new_turn=True) -> None or tuple:
        ...
        if allowed:
            ...
            self.move_history.append(
                {"ini_pos": ini_pos, "move": move, "piece": piece, "has_already_moved": piece.has_already_moved})
            ...
            if move.special_type == move.CASTLING_TYPE:
                ...
                rook = self.pieces_by_pos.pop((pos[0] + 1, pos[1])) if little_castling else self.pieces_by_pos.pop(
                    (pos[0] - 2, pos[1]))
            ...
            if call_new_turn:
                ...
                if isinstance(piece, Pawn) and ((piece.color == Piece.WHITE and piece.pos[1] == 0) or (
                        piece.color == Piece.BLACK and piece.pos[1] == 7)):
                    # if self.verbose >= 1: print(f"{piece} upgrading!")
                    new_piece_class = piece.promote_class_wanted
                    if new_piece_class is None:
                        warn(
                            f"No promote_class_wanted for {piece}, you should set the pawn's attribute promote_class_wanted before moving it\nWe'll use a Queen to promote it")
    ...
    def create_hypothesis_board(self, pieces_with_pos_to_change={}):
        ...
        for piece, new_pos in pieces_with_pos_to_change.items():
            if piece in real_piece_to_hypothesis_piece:
                hypo_board.move_piece(real_piece_to_hypothesis_piece[piece], new_pos, skip_allowed_verif=True,
                                      call_new_turn=False)
            else:
